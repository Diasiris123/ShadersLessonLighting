Shader "Unlit/ShaderFresnelHLSL" // shader name  - shader1 under shader category - Unlit
{
    // properties to be expose in the material
    Properties
    {
        _Power("Fresnel Power", Float) = 0.5
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
               ...
               float3 worldPos = mul(unity_ObjectToWorld, v.vertex);
                ...
                o.viewDirection = normalize(_WorldSpaceCameraPos - worldPos); //mul(UNITY_MATRIX_M, float4(v.vertex.xyz,1)); // from object to world pos
                //o.uv = v.uv;
                return o;
            }
            ...
            {
                float3 color = dot(i.viewDirection, i.normalWorldSpace);
                float4 finalColor = 1 - saturate(float4(color, 1));
